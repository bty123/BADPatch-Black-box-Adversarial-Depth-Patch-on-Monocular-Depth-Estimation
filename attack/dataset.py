...
class KittiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        img1_path = os.path.join(self.folder_path, img_name)
        img2_path = os.path.join(self.folder_path, img_name)
...
class CarlaDataset(Dataset):
    def __init__(self, model, main_dir, mode, transform=None):
        ...
        if self.mode == 'training':
            self.folder_path = os.path.join(self.main_dir, 'training')
        elif self.mode == 'testing':
            self.folder_path = os.path.join(self.main_dir, 'testing')
        else:
            raise ValueError("Mode must be 'training' or 'testing'")
...
class CarlaOwnDataset(Dataset):
    def __init__(self, model, main_dir, mode, transform=None):
        ...
        if self.mode == 'training':
            self.folder_path = os.path.join(self.main_dir, '320_1024_training_tesla_2.5')
        elif self.mode == 'testing':
            self.folder_path = os.path.join(self.main_dir, '320_1024_testing_tesla_2.5')
        else:
            raise ValueError("Mode must be 'training' or 'testing'")
